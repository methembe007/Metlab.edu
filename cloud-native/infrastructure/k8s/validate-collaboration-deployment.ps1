# Collaboration Service Deployment Validation Script (PowerShell)
...
Write-Host "Collaboration Service Deployment Validation" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    $null = kubectl get configmap collaboration-config -n $NAMESPACE 2>&1
    Write-Success "ConfigMap 'collaboration-config' exists"
    ...
    foreach ($key in $REQUIRED_KEYS) {
        try {
            $value = kubectl get configmap collaboration-config -n $NAMESPACE -o jsonpath="{.data.$key}" 2>&1
            if ($value) {
    ...
    Write-Error-Custom "ConfigMap 'collaboration-config' does not exist"
}
...
try {
    $null = kubectl get secret collaboration-secret -n $NAMESPACE 2>&1
    Write-Success "Secret 'collaboration-secret' exists"
    
    # Validate Secret keys
    $REQUIRED_SECRET_KEYS = @("DATABASE_PASSWORD", "REDIS_PASSWORD")
    foreach ($key in $REQUIRED_SECRET_KEYS) {
        try {
            $value = kubectl get secret collaboration-secret -n $NAMESPACE -o jsonpath="{.data.$key}" 2>&1
            if ($value) {
    ...
    Write-Error-Custom "Secret 'collaboration-secret' does not exist"
}
...
try {
    $null = kubectl get deployment $DEPLOYMENT_NAME -n $NAMESPACE 2>&1
...
try {
    $null = kubectl get service $SERVICE_NAME -n $NAMESPACE 2>&1
...
try {
    $null = kubectl get hpa collaboration-hpa -n $NAMESPACE 2>&1
    Write-Success "HPA 'collaboration-hpa' exists"
    ...
    Write-Warning-Custom "HPA 'collaboration-hpa' does not exist (optional)"
}
...
# Check PostgreSQL
try {
    $null = kubectl get service postgres -n $NAMESPACE 2>&1
...
# Check Redis
try {
    $null = kubectl get service redis -n $NAMESPACE 2>&1
    Write-Success "Redis service exists"
} catch {
    Write-Warning-Custom "Redis service not found (required dependency for pub/sub)"
}
...
if (![string]::IsNullOrWhiteSpace($FIRST_POD)) {
    ...
    try {
        $null = kubectl exec -n $NAMESPACE $FIRST_POD -- nc -zv postgres 5432 2>&1
    ...
    Write-Info "  Testing Redis connectivity from pod..."
    try {
        $null = kubectl exec -n $NAMESPACE $FIRST_POD -- nc -zv redis 6379 2>&1
        Write-Success "    Redis is reachable"
    } catch {
        Write-Warning-Custom "    Redis connectivity test failed"
    }
}
...
Write-Host ""
Write-Info "To test Redis pub/sub functionality:"
Write-Host "  kubectl exec -n $NAMESPACE -it $FIRST_POD -- redis-cli -h redis PING" -ForegroundColor Gray
