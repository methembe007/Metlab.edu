...
// Config holds the configuration for the homework service
type Config struct {
	Port           string
	DatabaseURL    string
	S3Endpoint     string
...
func LoadConfig() (*Config, error) {
	...
	s3Bucket := getEnv("S3_BUCKET", "metlab-homework")
	s3Region := getEnv("S3_REGION", "us-east-1")
	s3UseSSL := getEnv("S3_USE_SSL", "false") == "true"
	
	// Upload configuration
	maxUploadSizeStr := getEnv("MAX_UPLOAD_SIZE", "26214400") // 25MB default
